...
async def verify_checkout_payment(req: VerifyCheckoutRequest, user: dict = Depends(require_permission("write:create_order"))):
    ...
    Upon verification:
    - Links active_payment_id to payment_intent
    - Transitions payment state to AUTHORIZED
    - Enqueues resolution outbox task
    """
    from razorpay.webhooks import verify_payment_signature
    valid = verify_payment_signature(req.razorpay_order_id, req.razorpay_payment_id, req.razorpay_signature)
    if not valid:
        raise HTTPException(status_code=401, detail={"error": "invalid_checkout_signature", "message": "Razorpay checkout signature verification failed"})
    ...
    async with pool.acquire() as conn:
        ...
        if not intent:
            raise HTTPException(status_code=404, detail="Payment intent associated with order_id not found")
