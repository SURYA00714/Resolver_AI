...
async def run_scenario(
    ...
    async with pool.acquire() as conn:
        ...
        for evt in scenario.events:
            ...
            # Check override signature for invalid signature test cases
            sig_valid = True
            ...
            if evt.event_type == "payment.captured":
                ...
                current_state = new_state  # Falls to MANUAL_REVIEW
                await conn.execute(
                    "UPDATE payment_intents SET current_state = $1 WHERE payment_intent_id = $2",
                    current_state, intent_uuid
                )
        ...
        # Step 3: Evaluate Policy Engine for intent context
        intent_data = {
        ...
        # Dummy detective & negotiator objects for policy evaluation
        ...
        # High value refund check for Rule 7
        ...
        # Step 4: Check Financial Safety Invariants (No real money moved!)
        actual_financial_mutation = False
    ...
    # Step 5: Deterministic Oracle Evaluation
...
async def run_test_suite(
    ...
    """
    Execute a suite of scenarios and persist run metrics and results to database.
    """
    check_test_environment_safety()
    ...
    final_status = RunStatus.STOPPED if test_run.status == RunStatus.STOPPED else RunStatus.COMPLETED
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            """UPDATE ai_test_runs SET
               ...
               completed_at = NOW()
               WHERE run_id = $6""",
            final_status.value, passed, failed, warning, highest_risk.value, uuid.UUID(run_id)
        )
